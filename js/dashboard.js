...
function renderHheeExpenseChart(rows) {
    const selectedRoles = ROLE_DEFS;
    const maxTotal = Math.max(
        1,
        ...rows.map(row =>
            selectedRoles.reduce(
                (sum, role) => sum + (row.values[role.key] || 0),
                0
            )
        )
    );
    ...
    return `
        <div class="dashboard-bar-scroll">
            <div class="dashboard-stacked-bars" style="--dashboard-bars:${rows.length}">
                ${rows.map(row => {
                    const total = selectedRoles.reduce(
                        (sum, role) => sum + (row.values[role.key] || 0),
                        0
                    );

                    return `
                        <div class="dashboard-stacked-item" title="${escapeHTML(row.label)}: ${escapeHTML(formatMoney(total))}">
                            <div class="dashboard-stacked-bar" aria-label="${escapeHTML(row.label)} ${escapeHTML(formatMoney(total))}">
                                ${selectedRoles.map(role => {
                                    const amount = row.values[role.key] || 0;
                                    const height = (amount / maxTotal) * 100;

                                    return `
                                        <span style="height:${height}%; background:${role.color}" title="${escapeHTML(role.label)}: ${escapeHTML(formatMoney(amount))}"></span>
                                    `;
                                }).join("")}
                            </div>
        ...
        <div class="dashboard-chart-legend">
            ${selectedRoles.map(role => `
                <span><i style="background:${role.color}"></i>${escapeHTML(role.label)}</span>
            `).join("")}
        </div>
    `;
}
...
function renderLineChart(labels, series) {
    ...
    function point(value, index, count) {
        const x = pad.left + (
            count <= 1 ? 0 : (index / (count - 1)) * chartWidth
        );
        const y = pad.top + chartHeight -
            ((Number(value) || 0) / maxValue) * chartHeight;

        return `${x.toFixed(1)},${y.toFixed(1)}`;
    }

    if (!series.length) {
        return chartEmpty("No hay datos para graficar.");
    }

    return `
        <div class="dashboard-line-scroll">
            <svg class="dashboard-line-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Gráfico de líneas">
                ${ySteps.map(step => {
                    const y = pad.top + chartHeight - (step * chartHeight);
                    const value = Math.round(maxValue * step);

                    return `
                        <line class="dashboard-line-grid" x1="${pad.left}" y1="${y}" x2="${width - pad.right}" y2="${y}"></line>
                        <text class="dashboard-line-axis" x="8" y="${y + 4}">${value}</text>
                    `;
                }).join("")}
                ${labels.map((label, index) => {
                    ...
                    return `
                        <text class="dashboard-line-label" x="${x}" y="${height - 12}">${escapeHTML(label)}</text>
                    `;
                }).join("")}
                ${series.map(item => `
                    <polyline class="dashboard-line-path" points="${item.values.map((value, index) => point(value, index, labels.length)).join(" ")}" style="stroke:${item.color}"></polyline>
                    ${item.values.map((value, index) => {
                        const [x, y] = point(value, index, labels.length).split(",");

                        return `<circle class="dashboard-line-dot" cx="${x}" cy="${y}" r="3.4" style="fill:${item.color}"><title>${escapeHTML(item.label)} ${escapeHTML(labels[index])}: ${escapeHTML(formatNumber(value))}</title></circle>`;
                    }).join("")}
                `).join("")}
            </svg>
        </div>
...
function renderStaffingHeadcountChart(rows) {
    const series = ROLE_DEFS.map(role => ({
        label: role.label,
        color: role.color,
        values: rows.map(row => row.values[role.key] || 0)
    }));
