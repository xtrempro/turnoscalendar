// Activa o retira la escritura individual de reemplazos de una unidad.
...
if (
    !workspaceId ||
    !expectedName ||
    !["records-shadow-v1", "legacy"].includes(value)
) {
    throw new Error(
        "Usa --workspace, --expected-name y " +
        "--value records-shadow-v1|legacy."
    );
}
...
const previous = String(
    current.fields?.replacementStorage?.stringValue || ""
);
...
console.log(`replacementStorage: ${previous || "(sin marca)"} -> ${value}`);
...
patchUrl.searchParams.append("updateMask.fieldPaths", "replacementStorage");
const response = await fetch(patchUrl, {
    method: "PATCH",
    headers,
    body: JSON.stringify({
        fields: {
            replacementStorage: { stringValue: value }
        }
    })
});
...
console.log(
    "Aplicado: " +
    (result.fields?.replacementStorage?.stringValue || "(sin marca)")
);
