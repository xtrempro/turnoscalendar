...
const FIXTURE_ID = "turnoplus-test-unit-50-no-pwa-v1";
const WORKSPACE = {
    id: "test_unit_50_workers_no_pwa_v1",
    name: "[TEST] Unidad 50 Trabajadores Sin PWA"
};
...
// Distribución proporcional a la unidad de 60 (40/30/20/10 %).
function estamentoFor(index) {
    if (index < 20) return "Profesional";
    if (index < 35) return "Técnico";
    if (index < 45) return "Administrativo";
    return "Auxiliar";
}
...
function makeProfile(index) {
    ...
    const name =
        `${firstName} ${firstLastName} ${secondLastName} TEST50-${pad(number, 3)}`;

    return {
        profile: {
            id: `test50_${pad(number, 3)}`,
            name,
            email: `test.unit50.worker${pad(number, 3)}@example.invalid`,
            rut: formatRut(28000000 + number),
            phone: `92${pad(number, 6)}`,
            birthDate:
...
async function seed() {
    ...
    const writes = [
        writeDocument(`workspaces/${WORKSPACE.id}`, {
            ...
            synthetic: true,
            testDataNotice:
                "Unidad ficticia temporal con 50 trabajadores sin enlace PWA para pruebas"
        }),
        writeDocument(
        ...
        writeDocument(MANIFEST_PATH, {
            ...
            updatedAt: now,
            cleanupCommand:
                "node scripts/firebase-test-unit-50-fixture.mjs cleanup --confirm-test-cleanup"
        })
    ];
    ...
    if (result.profiles !== PROFILE_COUNT) {
        throw new Error("La verificación no coincide con 50 perfiles.");
    }
    if (result.pwaLinks !== 0) {
        throw new Error("Se esperaban 0 enlaces PWA en esta unidad.");
    }
}
